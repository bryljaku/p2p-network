...
class DownloadWorker {
    ...
    bool finished;
public:
    DownloadWorker(std::shared_ptr<Database> database1, std::shared_ptr<File> file1, Torrent torrent1, std::shared_ptr<PeerInfo> peer1):database(database1), file(std::move(file1)), torrent(std::move(torrent1)), peer(std::move(peer1)) {
